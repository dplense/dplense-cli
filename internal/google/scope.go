...
func ResolveScope(scope string, dirClient DirectoryClient, driveClient DriveClient, log logger.Logger) ([]Target, error) {
...
func resolveActiveUsers(ctx context.Context, dirClient DirectoryClient, log logger.Logger) ([]Target, error) {
	if dirClient == nil {
		return nil, fmt.Errorf("directory client is required for 'active' scope but is not available.\n\n" +
			...
			"  - shared-drives: ./dplense scan --scope shared-drives")
	}
...
func resolveSuspendedUsers(ctx context.Context, dirClient DirectoryClient, log logger.Logger) ([]Target, error) {
	if dirClient == nil {
...
func resolveSharedDrives(ctx context.Context, driveClient DriveClient, log logger.Logger) ([]Target, error) {
	if driveClient == nil {
...
func resolveUser(ctx context.Context, email string, dirClient DirectoryClient, log logger.Logger) ([]Target, error) {
